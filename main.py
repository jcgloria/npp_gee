...
def saveImage(image, year, month, locName):
    ...
    task = ee.batch.Export.image.toAsset(image=image,assetId='projects/ee-96juancg/assets/'+'npp_'+locName +'_'+str(month)+"_"+str(year), description='npp_'+locName+'_'+str(year),scale=500,region=area)
    task.start()
